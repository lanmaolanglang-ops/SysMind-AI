...
async def recent_actions(request: Request) -> ActionListResponse:
    return ActionListResponse(
        items=[ActionResponse.from_record(item) for item in _coordinator(request).recent()]
    )


@router.get("/{action_id}", response_model=ActionResponse)
async def get_action(action_id: str, request: Request) -> ActionResponse:
    record = _coordinator(request).get(action_id)
    if record is None:
...
async def confirm_action(action_id: str, request: Request) -> ConsentResponse:
    record, ticket, expires = _run(lambda: _coordinator(request).confirm(action_id))
...
async def reject_action(action_id: str, request: Request) -> ActionResponse:
    return ActionResponse.from_record(_run(lambda: _coordinator(request).reject(action_id)))
